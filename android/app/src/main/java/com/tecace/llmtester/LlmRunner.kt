...
class LlmRunner(private val context: android.content.Context) {
    ...
    suspend fun init(
        ...
        Log.d(TAG, ">>> Backend $preferredBackend: INIT SUCCESS (${initTimeMs}ms)")
    }

    suspend fun generate(prompt: String, inputTokenCount: Int): InferenceMetrics = withContext(Dispatchers.IO) {
