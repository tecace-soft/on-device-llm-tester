...
> **Project Status:** Phase 7 — Cloud Deployment ✅ (Vercel + Turso)
...
# DB 모드 (기본값: local)
# DB_MODE=local     → SQLite (로컬 개발)
# DB_MODE=turso     → Turso 클라우드 DB (Vercel 배포)
DB_MODE=local

# Turso 클라우드 DB (DB_MODE=turso 시 필수)
...
> **Cloud:** Vercel 배포 시 API는 `https://on-device-llm-tester.vercel.app/api/...` 경로로 서빙됩니다.
...
> **Cloud:** `https://on-device-llm-tester.vercel.app`
...
## 9. Cloud Deployment (Phase 7)

**Vercel** (React UI + FastAPI Serverless) + **Turso** (libSQL 클라우드 DB) 조합으로 배포합니다.

### 9.1 URLs

| 서비스 | URL |
|--------|-----|
| Dashboard | https://on-device-llm-tester.vercel.app |
| API Health | https://on-device-llm-tester.vercel.app/health |
| API Docs (로컬만) | http://localhost:8000/docs |
...
| `API_KEY` | 임의 문자열 (대시보드 인증 불필요 시 미설정) | 선택 |

### 9.3 CI Ingest (Turso 모드)

GitHub Actions에서 벤치마크 완료 후 Turso에 직접 적재:

```bash
DB_MODE=turso \
TURSO_URL=libsql://your-db.turso.io \
TURSO_AUTH_TOKEN=your_token \
python scripts/ingest.py --run-id <id> --trigger ci --branch main
```

### 9.4 로컬 개발

Vercel 없이 로컬에서 그대로 동작합니다 — `DB_MODE=local`(기본값)이면 SQLite 사용:

```bash
cd api && uvicorn main:app --reload        # API :8000
cd dashboard && npm run dev               # UI :5173
```

상세 배포 아키텍처: [`DEPLOYMENT_ARCHITECTURE.md`](DEPLOYMENT_ARCHITECTURE.md)
...
| `DEPLOYMENT_ARCHITECTURE.md` | **Phase 7 클라우드 배포 설계 (Vercel + Turso)** |
...
| 7 | Cloud Deployment (Vercel + Turso) | ✅ |
